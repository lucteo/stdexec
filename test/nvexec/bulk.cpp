...
namespace {
  ...
  TEST_CASE("nvexec bulk returns a sender", "[cuda][stream][adaptors][bulk]") {
    ...
    auto snd = ex::bulk(ex::schedule(stream_ctx.get_scheduler()), ex::par, 42, [] { });
  ...
  TEST_CASE(
    ...
    auto task =
      ...
      | stdexec::bulk(ex::par, nelems, [](std::size_t i, std::span<double> out) { out[i] = i; })
      | stdexec::let_value([](std::span<double> out) { return stdexec::just(out); })
      | stdexec::bulk(ex::par, nelems, [](std::size_t i, std::span<double> out) {
          out[i] = 2.0 * out[i];
        });
